...
export function AddKeySheet({ open, projectId, branchId, locales, existingKeys, onClose, onCreated }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose() }}>
      <DialogContent className="max-w-3xl p-0 bg-background border-border flex flex-col max-h-[90vh] [&>button]:hidden">
        <DialogHeader className="px-6 py-4 border-b border-border flex-shrink-0 flex flex-row items-center justify-between">
          <DialogTitle className="text-foreground text-base">Add Translation Key</DialogTitle>
          <button type="button" onClick={handleClose} className="text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>
        </DialogHeader>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          >
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                className="font-mono text-sm bg-card border-border focus:border-blue-500"
                autoFocus
              />
              {keyError && <p className="text-[11px] text-destructive">{keyError}</p>}
              {!keyError && (
                <p className="text-[11px] text-muted-foreground">Lowercase letters, numbers, dots, underscores</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                maxLength={500}
                rows={2}
                className="w-full text-sm bg-card border border-border rounded-md px-3 py-2 text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-blue-500 resize-none"
              />
            </div>
            ...
            {baseLocale && (
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">
                  Base value <span className="text-muted-foreground">({baseLocale.code.toUpperCase()})</span>
                </label>
                <Input
                  value={baseValue}
                  onChange={(e) => setBaseValue(e.target.value)}
                  placeholder={`Enter ${baseLocale.name} translation…`}
                  className="text-sm bg-card border-border focus:border-blue-500"
                />
              </div>
            )}
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={[
                    ...
                    charLimitEnabled ? 'bg-blue-600' : 'bg-accent',
                  ].join(' ')}
                >
                  <span
                    className={[
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="w-6 h-6 rounded bg-muted border border-border flex items-center justify-center text-muted-foreground hover:text-foreground"
                      >
                      ...
                      <Input
                        ...
                        className="w-16 text-center text-sm bg-card border-border h-7 px-1"
                        min={1}
                        max={9999}
                      />
                      <button
          ...
          {/* Right: preview */}
          <div className="w-52 flex-shrink-0 border-l border-border bg-card/50 overflow-y-auto p-4 space-y-4">
            <div className="text-[11px] font-medium text-muted-foreground uppercase tracking-wide">Preview</div>

            {/* Summary card */}
            <div className="rounded-md border border-border bg-card p-3 space-y-1.5">
              {keyName ? (
                <p className="font-mono text-[11px] text-blue-700 dark:text-blue-300 break-all">{keyName}</p>
              ) : (
                <p className="text-[11px] text-muted-foreground italic">Enter a key name…</p>
              )}
              {description && (
              ...
              {tags.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {tags.map((t) => (
                    <span key={t} className="text-[10px] bg-accent text-foreground px-1.5 rounded">{t}</span>
                  ))}
                </div>
              )}
              {platforms.length > 0 && (
                <p className="text-[10px] text-muted-foreground">{platforms.join(' · ')}</p>
              )}
              {charLimitEnabled && charLimit && (
                <p className="text-[10px] text-muted-foreground">Max {charLimit} chars</p>
              )}
            </div>

            {/* JSON preview */}
            <div>
              <div className="text-[11px] font-medium text-muted-foreground mb-1.5">JSON output</div>
              <pre className="text-[10px] font-mono text-foreground bg-background rounded border border-border p-2 overflow-x-auto whitespace-pre-wrap break-all">
                {jsonPreview}
              </pre>
            </div>
          </div>
