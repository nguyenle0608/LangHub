export default function EditorLoading() {
  return (
    <div className="flex flex-col h-screen bg-background text-foreground overflow-hidden">
      ...
      {/* Table header */}
      <div className="flex items-center gap-0 border-b border-border bg-card/60 px-4 h-9 flex-shrink-0">
        <div className="w-5 h-4 bg-muted rounded animate-pulse mr-3" />
        <div className="w-44 h-4 bg-muted rounded animate-pulse mr-6" />
        {[1, 2, 3].map((i) => (
      ...
      <div className="flex-1 overflow-hidden">
        {Array.from({ length: 12 }, (_, i) => (
          <div
            key={i}
            className="flex items-center gap-0 border-b border-border/50 px-4 h-12"
            style={{ opacity: 1 - i * 0.07 }}
          >
            <div className="w-5 h-4 bg-muted/60 rounded animate-pulse mr-3" />
